# 0004-export-axi-bus-irqs-with-microvm-acpi.patch

This patch exports `axi-bus` windows through microvm ACPI so Linux maps the
...
During DSDT construction, QEMU scans dynamic sysbus devices for `axi-bus`
...
the QEMU `axi-bus` transport proxy; external backends continue to own virtio
device semantics.
