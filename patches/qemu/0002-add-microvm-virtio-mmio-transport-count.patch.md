...
This project needs QEMU to expose only the custom `axi` MMIO window and to
...
## Reserved axi IRQ Range

The patch reserves primary IO-APIC GSIs `16..23` for project `axi`
devices:

```text
MICROVM_AXI_IRQ_BASE  = 16
MICROVM_AXI_IRQ_COUNT = 8
```
...
```text
machine_class_allow_dynamic_sysbus_dev(mc, "axi")
```

Without this allowlist entry, QEMU would reject `-device axi,...` on the
`microvm` machine.

## Runtime Relationship

This patch does not instantiate `axi`. It only lets the launcher request it
