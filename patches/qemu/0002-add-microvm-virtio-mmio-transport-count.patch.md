...
This project needs QEMU to expose only the custom `axi-bus` MMIO window and to
...
## Reserved axi-bus IRQ Range

The patch reserves primary IO-APIC GSIs `16..23` for project `axi-bus`
devices:

```text
MICROVM_AXI_BUS_IRQ_BASE  = 16
MICROVM_AXI_BUS_IRQ_COUNT = 8
```
...
Without this allowlist entry, QEMU would reject `-device axi-bus,...` on the
`microvm` machine.

## Runtime Relationship

This patch does not instantiate `axi-bus`. It only lets the launcher request it
and disables unrelated built-in transport slots.

The actual device still comes from:

```text
-device axi-bus,id=blk0,base=...,size=...,irq=16,socket=...
```
